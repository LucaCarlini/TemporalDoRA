...
class DoRALinear(nn.Linear):
    """
    Paper-faithful DoRA reparameterization:
      W' = m * (W0 + BA) / ||W0 + BA||_c
    where ||.||_c is the column-wise norm and m is initialized as ||W0||_c.
    """
    ...
    def __init__(
        ...
        bias: bool = False,
        eps: float = 1e-6,
        dora_rank: Optional[int] = None,
        detach_norm: bool = True,
    ):
        super().__init__(in_features, out_features, bias=bias)
        self.eps = eps
        self.detach_norm = bool(detach_norm)
        self.dora_rank = int(dora_rank) if dora_rank is not None else int(in_features)
        if self.dora_rank <= 0:
            raise ValueError(f"dora_rank must be > 0, got {self.dora_rank}.")

        # W0 is the frozen base direction component.
        ...
        # Delta-V = B @ A (LoRA-style directional update).
        ...
        # m = ||W0||_c (column-wise norm), as in the original DoRA initialization.
        with torch.no_grad():
            m0 = torch.linalg.vector_norm(self.weight, dim=0)
    ...
    def from_linear(
        ...
        with torch.no_grad():
            module.weight.copy_(base_layer.weight.detach())
            if module.bias is not None and base_layer.bias is not None:
                module.bias.copy_(base_layer.bias.detach())
            module.magnitude.copy_(torch.linalg.vector_norm(module.weight, dim=0))
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        delta_v = torch.matmul(self.lora_B, self.lora_A)
        v_prime = self.weight + delta_v
        col_norm = torch.linalg.vector_norm(v_prime, dim=0, keepdim=True).clamp_min(self.eps)
        if self.detach_norm:
            col_norm = col_norm.detach()
        scaled_weight = (v_prime / col_norm) * self.magnitude.unsqueeze(0)
...
def _build_encoder_dora_base(
    ...
    module = DoRALinear.from_linear(base_layer, dora_rank=rank)
    module.weight.requires_grad_(False)
    if module.bias is not None:
        module.bias.requires_grad_(False)
    ...
    return module.to(device=base_layer.weight.device, dtype=base_layer.weight.dtype)
...
class TimeShiftedVisionLinear(nn.Module):
    ...
    def __init__(
        ...
        self.use_dora = bool(use_dora)
        self.base_dora = _build_encoder_dora_base(base_layer, self.use_dora, int(rank))
        ...
        self.adapter = TimeShiftLoRA(
            ...
            # Encoder stdora should apply DoRA on the pretrained base projection.
            # Keep the temporal branch as the LoRA-style directional adapter.
            use_dora=False,
        )
    ...
    def forward(self, x):
        base_out = self.base_dora(x) if self.base_dora is not None else self.base_layer(x)
        if x.dim() == 2:
...
class STLoRAVisionLinear(nn.Module):
    ...
    def __init__(
        ...
        self.stlora = STLoRA3DInline(
            ...
            # Encoder stdora keeps DoRA on base projection and LoRA-style temporal branch.
            use_dora=False,
        )
...
class STAttentionLoRAVisionLinear(nn.Module):
    __slots__ = ("base_layer", "base_dora", "adapter", "num_frames", "layer_type", "attention_cls")
