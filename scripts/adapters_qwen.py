...
def _chunk_qwen_visual_tokens(
    ...
    if offset != main_tokens.size(0):
        # Some call sites (e.g., pre-MHSA adapters) may receive unmerged tokens even when
        # visual.spatial_merge_size > 1. Retry with merge=1 if it exactly matches token length.
        if spatial_merge_size > 1:
            unmerged_total = 0
            for entry in grid:
                t, h, w = [max(1, int(v)) for v in entry]
                unmerged_total += t * h * w
            if unmerged_total == main_tokens.size(0):
                return _chunk_qwen_visual_tokens(main_tokens, grid, spatial_merge_size=1)
        raise RuntimeError(
...
def _run_qwen_st_module(module: nn.Module, chunks: List[Dict[str, Any]]) -> torch.Tensor:
    ...
    # PEFT can wrap custom modules via ModulesToSaveWrapper; dispatch on the active inner module.
    resolved_module = module
    modules_to_save = getattr(module, "modules_to_save", None)
    active_adapter = getattr(module, "active_adapter", None)
    if modules_to_save is not None and active_adapter is not None:
        try:
            candidate = modules_to_save[active_adapter]
        except Exception:
            candidate = None
    ...
    module_hidden = getattr(resolved_module, "hidden_size", None)
    ...
    if isinstance(resolved_module, SpatioTemporalAdapterModule):
        ...
        for chunk in chunks:
            ...
            seq = tokens.unsqueeze(0)  # (1, T, HW, C)
            out = resolved_module(
                seq,
                num_frames=t,
            )
            if out.shape != seq.shape:
                raise ValueError(
                    f"[ST-Adapter] Expected output shape {tuple(seq.shape)}, got {tuple(out.shape)}."
                )
            restored.append(out[0].reshape(t * spatial, hidden))
        return torch.cat(restored, dim=0)

    if isinstance(resolved_module, TimeShiftLoRA):
        restored: List[torch.Tensor] = []
        for chunk in chunks:
            tokens = chunk["tokens"]  # (T, HW, C)
            t = tokens.shape[0]
            ...
            out_btpc = resolved_module(tokens, num_frames=t)
            if out_btpc.dim() != 3:
    ...
    for chunk in chunks:
        ...
        if isinstance(resolved_module, (STMultiHeadAttentionLoRAInline, STSelfAttentionLoRAInline)):
            out_btpc = resolved_module(
                seq,
                num_frames=t,
                grid_hw=(h, w),
            )
...
class QwenPreMHSAAdapter(nn.Module):
    """
    Wraps a visual attention module and injects an ST-Adapter residual before MHSA.
    Exposes qkv/proj so existing Qwen attention traversal helpers keep working.
    """
    ...
    def __init__(
        ...
        self.spatial_merge_size = max(1, int(spatial_merge_size))
    ...
    def forward(self, hidden_states: torch.Tensor, *args, **kwargs):
        if not isinstance(hidden_states, torch.Tensor) or hidden_states.dim() != 2:
            return self.base_attn(hidden_states, *args, **kwargs)

        parent = self._parent_ref() if self._parent_ref is not None else None
        grid_thw = getattr(parent, "_st_last_grid_thw", None) if parent is not None else None
        delta = _compute_st_delta(
            self.adapter,
            hidden_states,
            grid_thw=grid_thw,
            spatial_merge_size=self.spatial_merge_size,
        )
        return self.base_attn(hidden_states + delta, *args, **kwargs)
...
def _collect_qwen_encoder_modules_to_save(model: nn.Module) -> List[str]:
    ...
    for parent_name, attn_attr, attn in _iter_qwen_visual_attn_layers_ex(model, strict=False):
        if isinstance(attn, QwenPreMHSAAdapter):
            paths.append(f"{visual_path}.{parent_name}.{attn_attr}.adapter")
...
def _ensure_qwen_vision_encoder_adapters_trainable(model: nn.Module) -> bool:
    ...
    for _parent_name, _attn_attr, attn in _iter_qwen_visual_attn_layers_ex(model, strict=False):
        if isinstance(attn, QwenPreMHSAAdapter):
            for p in attn.adapter.parameters():
                if not p.requires_grad:
                    p.requires_grad = True
                    updated = True
            for p in attn.base_attn.parameters():
                if p.requires_grad:
                    p.requires_grad = False
                    updated = True
        for proj_attr in ("qkv", "proj"):
...
def install_qwen_st_adapter_before_mhsa(model, args) -> None:
    requested_hidden = int(getattr(args, "st_hidden", 0) or 0)
    _ensure_qwen_visual_hook(model)

    base_model, _ = _resolve_qwen_base_model(model)
    visual = base_model.visual

    attn_layers = list(_iter_qwen_visual_attn_layers_ex(model, strict=True))
    attn_hidden_size: Optional[int] = None
    for _parent_name, _attn_attr, attn in attn_layers:
        qkv = getattr(attn, "qkv", None)
        in_features = getattr(qkv, "in_features", None)
        if isinstance(in_features, int) and in_features > 0:
            attn_hidden_size = int(in_features)
            break
        proj = getattr(attn, "proj", None)
        proj_in = getattr(proj, "in_features", None)
        if isinstance(proj_in, int) and proj_in > 0:
            attn_hidden_size = int(proj_in)
            break
    if attn_hidden_size is None:
        attn_hidden_size = infer_visual_hidden_size(model, requested_hidden or None)

    if requested_hidden > 0 and requested_hidden != attn_hidden_size:
        print(
            f"[ST-Adapter-PreMHSA] Overriding --st_hidden={requested_hidden} with "
            f"attention input hidden size {attn_hidden_size}."
        )
    hidden_size = attn_hidden_size

    ref_param = next(model.parameters())
    parent_ref = weakref.ref(model)
    # Pre-MHSA attention inputs are patch tokens before the visual merger; chunk as unmerged.
    spatial_merge_size = 1
    installed_modules: List[nn.Module] = []
    wrapped = 0

    for parent_name, attn_attr, attn in attn_layers:
        if isinstance(attn, QwenPreMHSAAdapter):
            continue
        ...
        wrapped_attn = QwenPreMHSAAdapter(
            base_attn=attn,
            adapter_module=adapter,
            parent_ref=parent_ref,
            spatial_merge_size=spatial_merge_size,
        )
        parent_module = visual.get_submodule(parent_name) if parent_name else visual
        setattr(parent_module, attn_attr, wrapped_attn)
        installed_modules.append(adapter)
        wrapped += 1

    if wrapped == 0:
        raise RuntimeError("[ST-Adapter-PreMHSA] No visual attention modules were wrapped.")
    ...
    print(
        f"[ST-Adapter-PreMHSA] Installed before MHSA in {wrapped} visual blocks "
        f"(hidden={hidden_size}, channels={args.st_adapter_channels}, "
        f"kernel=({args.st_adapter_kernel_t}, {args.st_adapter_kernel_h}, {args.st_adapter_kernel_w}), "
        f"disable_cudnn={args.st_adapter_disable_cudnn})."
    )
...
def setup_adapter(model, args):
    ...
    if peft_enc in {
        ...
        install_qwen_st_adapter_before_mhsa(model, args)
        enable_vision_grad = True
