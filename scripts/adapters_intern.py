...
_STANDARD_ALIAS_MAP: Dict[str, str] = {
    # Encoder aliases
    "stlora_standard_enc": "stlora_enc",
    "stdora_standard_enc": "stdora_enc",
    "stlora_standard_mhatt_enc": "stlora_mhatt_enc",
    "stdora_standard_mhatt_enc": "stdora_mhatt_enc",
    "stlora_standard_selfatt_enc": "stlora_selfatt_enc",
    "stdora_standard_selfatt_enc": "stdora_selfatt_enc",
    "stlora_standard_lstm_enc": "stlora_lstm_enc",
    "stdora_standard_lstm_enc": "stdora_lstm_enc",
    "stmamba_standard_enc": "stmamba_enc",
    "stdora_standard_mamba_enc": "stdora_mamba_enc",
    # Decoder aliases
    "stlora_standard": "stlora",
    "stdora_standard": "stdora",
    "stlora_standard_mhatt": "stlora_mhatt",
    "stdora_standard_mhatt": "stdora_mhatt",
    "stlora_standard_selfatt": "stlora_selfatt",
    "stdora_standard_selfatt": "stdora_selfatt",
    "stlora_standard_lstm": "stlora_lstm",
    "stdora_standard_lstm": "stdora_lstm",
    "stmamba_standard": "stmamba",
    "stdora_standard_mamba": "stdora_mamba",
}


def _normalize_standard_alias(name: str) -> str:
    key = (name or "").lower()
    return _STANDARD_ALIAS_MAP.get(key, key)
...
def _infer_intern_visual_hidden_size(model: nn.Module, fallback: Optional[int] = None) -> int:
    vision_model = _require_intern_vision_model(model)
    for attr in ("hidden_size", "embed_dim", "dim"):
        value = getattr(vision_model, attr, None)
    ...
    if isinstance(layers, nn.ModuleList) and len(layers) > 0:
        first_attn = getattr(layers[0], "attn", None)
        qkv = getattr(first_attn, "qkv", None) if first_attn is not None else None
        in_features = getattr(qkv, "in_features", None)
        if isinstance(in_features, int) and in_features > 0:
            return int(in_features)

    if isinstance(fallback, int) and fallback > 0:
        return int(fallback)
    raise RuntimeError("[ST-Adapter][InternVL] Unable to infer visual hidden size.")


class InternPreMHSAAdapter(nn.Module):
    """
    Wraps InternVL vision attention and injects ST-Adapter residual before MHSA.
    Exposes qkv/proj so existing helper logic can still discover wrapped projections.
    """
    ...
    def __init__(
        ...
        parent_ref: "weakref.ReferenceType[nn.Module]",
    ) -> None:
        super().__init__()
    ...
    @property
    def qkv(self):
        return getattr(self.base_attn, "qkv", None)

    @qkv.setter
    def qkv(self, value) -> None:
        setattr(self.base_attn, "qkv", value)

    @property
    def proj(self):
        return getattr(self.base_attn, "proj", None)

    @proj.setter
    def proj(self, value) -> None:
        setattr(self.base_attn, "proj", value)

    def _resolve_num_frames(self) -> int:
        parent = self._parent_ref() if self._parent_ref is not None else None
        if parent is None:
            return 8
        value = getattr(parent, "stadapter_num_frames", None)
        if isinstance(value, int) and value > 0:
            return int(value)
        return 8

    def _apply_adapter(self, hidden_states: torch.Tensor) -> torch.Tensor:
        if hidden_states.dim() != 3:
            return hidden_states

        num_frames = self._resolve_num_frames()
        if num_frames <= 0:
            raise ValueError("[ST-Adapter][InternVL] num_frames must be > 0.")

        batch_like, token_len, hidden = hidden_states.shape

        if batch_like % num_frames == 0:
            batch_size = batch_like // num_frames
            seq = hidden_states.reshape(batch_size, num_frames, token_len, hidden)
            out = self.adapter(seq, num_frames=num_frames)
            return out.reshape(batch_like, token_len, hidden)

        if token_len % num_frames == 0:
            tokens_per_frame = token_len // num_frames
            seq = hidden_states.reshape(batch_like, num_frames, tokens_per_frame, hidden)
            out = self.adapter(seq, num_frames=num_frames)
            return out.reshape(batch_like, token_len, hidden)

        raise ValueError(
            f"[ST-Adapter][InternVL] Cannot align shape {tuple(hidden_states.shape)} "
            f"with num_frames={num_frames}."
        )

    def forward(self, hidden_states: torch.Tensor, *args, **kwargs):
        processed = self._apply_adapter(hidden_states)
        return self.base_attn(processed, *args, **kwargs)
...
def install_intern_st_adapter_before_mhsa(model, args, num_frames: int) -> None:
    hidden_override = args.st_hidden if getattr(args, "st_hidden", 0) > 0 else None
    hidden_dim = _infer_intern_visual_hidden_size(model, fallback=hidden_override)
    ...
    if layers is None:
        raise AttributeError("[ST-Adapter][InternVL] vision_model.encoder has no layers.")
    if len(layers) == 0:
        raise RuntimeError("[ST-Adapter][InternVL] vision_model.encoder has no layers.")
    ...
    for idx, layer in enumerate(layers):
        ...
        if attn is None:
            raise AttributeError(f"[ST-Adapter][InternVL] Layer {idx} missing attn module.")
        if isinstance(attn, InternPreMHSAAdapter):
            continue

        qkv = getattr(attn, "qkv", None)
        proj = getattr(attn, "proj", None)
        if not isinstance(qkv, nn.Module) or not isinstance(proj, nn.Module):
            raise AttributeError(
                f"[ST-Adapter][InternVL] Layer {idx} attention is missing qkv/proj modules."
            )

        adapter_module = SpatioTemporalAdapterModule(
            hidden_size=hidden_dim,
            adapter_channels=int(getattr(args, "st_adapter_channels", 384)),
            kernel_size=(
                int(getattr(args, "st_adapter_kernel_t", 3)),
                int(getattr(args, "st_adapter_kernel_h", 1)),
                int(getattr(args, "st_adapter_kernel_w", 1)),
            ),
            disable_cudnn=bool(getattr(args, "st_adapter_disable_cudnn", False)),
        ).to(device=ref_param.device, dtype=ref_param.dtype)

        for p in adapter_module.parameters():
            p.requires_grad = True

        wrapped_attn = InternPreMHSAAdapter(
            base_attn=attn,
            adapter_module=adapter_module,
            parent_ref=parent_ref,
        )
        layer.attn = wrapped_attn
        installed_modules.append(adapter_module)
        modules_to_save.append(f"{prefix_root}.layers.{idx}.attn.adapter")
        wrapped += 1

    if wrapped == 0:
        raise RuntimeError("[ST-Adapter][InternVL] No vision attention layers were wrapped.")

    model._before_attn_adapter_modules = nn.ModuleList(installed_modules)
    model._modules_to_save_paths = sorted(set(modules_to_save))
    model.stadapter_module = None
    model.stadapter_num_frames = int(num_frames)

    total = sum(p.numel() for p in model.parameters())
    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(
        f"[ST-Adapter][InternVL] Installed before MHSA in {wrapped} vision layers. "
        f"Trainable={trainable:,} / Total={total:,} "
        f"(hidden={hidden_dim}, channels={int(getattr(args, 'st_adapter_channels', 384))}, "
        f"kernel=({int(getattr(args, 'st_adapter_kernel_t', 3))}, "
        f"{int(getattr(args, 'st_adapter_kernel_h', 1))}, "
        f"{int(getattr(args, 'st_adapter_kernel_w', 1))}))."
    )
...
def _collect_intern_encoder_modules_to_save(model: nn.Module) -> List[str]:
    ...
    for idx, layer in enumerate(layers):
        ...
        if isinstance(attn, InternPreMHSAAdapter):
            paths.append(f"{prefix_root}.layers.{idx}.attn.adapter")
...
def _ensure_vision_encoder_adapters_trainable(model: nn.Module) -> bool:
    ...
    for layer in layers:
        ...
        if attn is None:
            raise AttributeError("[Adapter][InternVL] Missing attn in vision encoder layer.")
        if isinstance(attn, InternPreMHSAAdapter):
            for param in attn.adapter.parameters():
                if not param.requires_grad:
                    param.requires_grad = True
                    updated = True
            for param in attn.base_attn.parameters():
...
def setup_adapter(model, args):
    ...
    peft_enc = _normalize_standard_alias(args.peft_enc)
    peft_dec = _normalize_standard_alias(args.peft_dec)
    ...
    elif peft_enc in {"st_adapter_enc"}:
        install_intern_st_adapter_before_mhsa(
            model,
            args,
            num_frames=adapter_frames,
        )
