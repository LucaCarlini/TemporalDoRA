...
TemporalDoRA is a parameter-efficient temporal adaptation approach for surgical VideoVQA.
...
![TemporalDoRA architecture](figs/temporaldora_schema_shaded.png)
...
To re-run the TemporalDoRA experiment from the paper, use `--peft_enc stdora_mhatt_enc` with `--peft_dec dora`.
...
- `standard` variants indicate DoRA with an added mid-layer in the adapter bottleneck (for example `stdora_standard_enc`, `stdora_standard_mhatt_enc`, `stdora_standard`).
